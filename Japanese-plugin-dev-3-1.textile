...
h2. ファンクションタグ プラグイン（&lt;MTHelloWorld2&gt;）の開発（Perl）
...
h2. &lt;MTHelloWorld2&gt;の動作確認

h3. ブログ記事テンプレートにタグを追加
...
Config Directiveとの使い分けですが、先にも書きましたが一度設定してしまえばシステム全体で利用される物をConfig Directive、システム構築後に設定変更する可能性のある物はプラグイン設定画面を用意するという使い分けがよいでしょう。
