...
|sub getElementsByTagName {}|タグ名でDOMエレメントを検索|
|sub getElementsByClassName {}|クラス名でDOMエレメントを検索|
|sub getElementsByName {}|nameでDOMエレメントを検索|
|sub getElementById {}|idでDOMエレメントを検索|
|sub createElement {}|新規ノードを作成|
|sub createTextNode {}|新規テキストノードの作成|
|sub insertAfter {}|指定されたDOMエレメントの後にエレメントを挿入|
|sub insertBefore {}|指定されたDOMエレメントの前にエレメントを挿入|
|sub childNodes {}|指定された物の子ノードを取得|
|sub hasChildNodes {}|子ノードの有無を確認|
|sub appendChild {}|指定されたノードに子ノードを追加|

取得したノードの操作のために、MT::Template::Nodeに関数が用意されています。
